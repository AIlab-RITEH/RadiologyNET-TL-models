...
class model_training_app:
    ...
    def init_augumentation_model(self):
        ...
        if self.model_params.augumentation_model == 'COVID19_RGB':
            self.aug_model = TransformRGB_COVID19()
            self.aug_model_color_only = TransformToRGB() 

        if self.model_params.augumentation_model == 'COVID19_GRAY':
            self.aug_model = TransformGray_COVID19()
            self.aug_model_color_only = AddChanels()
    ...
    def save_model(self, epoch, best, info):
        ...
        _name = f"{self.model_params.name}_{self.model_params.opt_name}:" +  \
                    ...
                    f"_{epoch}"
        _name = 'last_model_checkpoint'  # to save some memory
        _model = self.model
