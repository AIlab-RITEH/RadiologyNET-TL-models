...
my_cache = get_cache('COVID19TEST')
...
class COVID_dataset:
    ...
    def __init__(self, dataset_config: cfg.datasetConfig):
        ...
        assert os.path.exists(self.dataset_config.covid_imgs_png_home_path), f"Path {self.dataset_config.covid_imgs_png_home_path} does not exist"
        assert os.path.exists(self.dataset_config.normal_imgs_png_home_path), f"Path {self.dataset_config.normal_imgs_png_home_path} does not exist"

        # Obtain data lists --> all data as json
        _data_list_train = self.obtain_structured_data()
        
        # Create splits
        _data_list_splitted = self.split_into_sets(_data_list_train, split_ratio = self.dataset_config.split_ratio, undersample=True)   
    ...
    def obtain_structured_data(self)->dict:
        ...
        """   
        # Input path NORMAL
        _input_path = self.dataset_config.normal_imgs_png_home_path
    
        # Save list
        _data_list = []
    
        # Obtain all files
        _files = os.listdir(_input_path)
        # Go through labels and build a list
        for _file in _files:
            _item = {'img_path': os.path.join(_input_path,_file), 
                     'label': 0}
            _data_list.append(_item)

        # Input path COVID
        _input_path = self.dataset_config.covid_imgs_png_home_path
...
def init_dataloader(data_loader_params: cfg.loaderConfig, dataset_params: cfg.datasetConfig)->DataLoader:
    """
        ...
        Output:
            * Torch DataLoader
    """
    _ds = COVID_dataset(dataset_params)
