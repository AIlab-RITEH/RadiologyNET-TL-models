...
class model_training_app:
    ...
    def init_augumentation_model(self):
        ...
        if self.model_params.augumentation_model == 'GRAZPEDWRI_RGB':
            self.aug_model = TransformRGB_GRAZPEDWRI()
            self.aug_model_color_only = TransformToRGB() 

        if self.model_params.augumentation_model == 'GRAZPEDWRI_GRAY':
            self.aug_model = TransformGray_GRAZPEDWRI()
            self.aug_model_color_only = AddChanels()
    ...
    def train_one_epoch(self, data):
        ...
        for _index, _batch in _batch_iter:
            ...
            # Calc loss
            _loss = self.get_bc_entropy_loss(_index, _batch, _predictions, _true, True)
    ...
    def validate_model(self, data):
        ...
        with torch.no_grad():
            ...
            for _index, _batch in enumerate(data):
                # Get loss
                _loss = self.get_bc_entropy_loss(_index, _batch, _predictions, _true, False)
    ...
    # Crossentropy loss
    #*******************************************************#
    def get_bc_entropy_loss(self, index, batch, predictions, true, augumentation = True):
        """
        ...
        # For metrics
        with torch.no_grad():
            _prediction_arg_max = (_prediction > 0.5).int().squeeze(1)
            _output_data = _output_data.int().squeeze()
    ...
    def save_model(self, epoch, best, info):
        ...
        _name = 'last_model_checkpoint.pth' # to preserve memory
