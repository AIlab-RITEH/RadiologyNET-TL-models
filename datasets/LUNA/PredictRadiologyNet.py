...
def plot_first_image_from_batch(batch, ax):
	...
	elem = dataset.Luna2dSegmentationItem(series_uid=_seriesuids[0], ct=_CTs[0],  pos=_pos_masks[0], slice_index=_slice_idxs[0])
...
#	'vgg16'
	...
	_lr = 0
	_info = "RadiologyNet_"+str(_lr)+"_"+_backbone
	print(f'Running info {_info}')
	...
	for _dir in _final_directories_list:
		...
		for _model in _models_list:
			...
			_training.model_predict_from_dl(val_dl, 'MiddleMan2')
		
			# Work on statistics
			# Obtain files in MiddleMan
			_result_files = os.listdir('MiddleMan2')
			...
			for _image_file in _result_files:
				_true_mask_path = "MiddleMan2"+"/"+_image_file+"/"+"GT_mask.png" 
				_predicted_mask_path = "MiddleMan2"+"/"+_image_file+"/" + "PRED_mask.png"
