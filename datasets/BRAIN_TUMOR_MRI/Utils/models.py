...
def freeze_model_part(model, freeze_ratio:float):
    ...
    logger.info(f"Freezing ratio: {freeze_ratio}")
...
def transfer_weights_to_model(path: str, target_model, device = 'cpu', verbose:bool=False):
    """
    ...
    for _src_name, _src_param in _src_state_dict['model_state'].items():
        ...
        if _src_name in _target_state_dict:
            ...
            if _src_param.shape == _target_state_dict[_src_name].shape:
                ...
                logger.debug(f"UNABLE TRANSFER at layer: {_src_name}/{_src_param.shape}")
        else:
            logger.debug(f"LAYER NOT FOUND: {_src_name}")
        
        # Expand for ImageNet pretrained models
        _expanded_name = "model."+_src_name
        logger.debug(f"TRYING with expaded name: {_expanded_name}")
...
class AddChanels:
    """
    Class which simply transform 1 channel to 3 channel gray image
    """
    def __init__(self):
        pass
